...
if not check_authentication():
    ...
    with st.sidebar:
        ...
        if user:
            st.write(f"**Usuario:** {user.get('nombre', 'N/A')}")
            st.write(f"**Rol:** {user.get('rol', 'N/A').capitalize()}")
        
        st.markdown("---")
        if st.button("🚪 Cerrar Sesión"):
            logout()
            # No need to rerun here, logout() handles it.

    # --- Original content of Home.py ---
    st.markdown('''
    <style>
        :root {
            --cian: #00F3FF;
    ...
    with col2:
        st.markdown('<div class="metric-card alert-critical"></div>', unsafe_allow_html=True)
